...
The main script for combining and preparing data for analysis is:
[`R/process-data.R`](R/process-data.R). The following R scripts were
used to process the data stored here:
...
- [`R/utils-variants.R`](R/utils-variants.R) : Takes raw data on variant
  sequencing stored here, cleans and classifies phases of dominant
  variants
  - Inputs:
    - `variants/eu-ecdc.csv`: 30 EU countries covered by the ECDC,
      source:
      <https://www.ecdc.europa.eu/en/publications-data/data-covid-19-variants-eueea>
    - `variants/uk-ukhsa.csv`: United Kingdom, source:
      <https://ukhsa-dashboard.data.gov.uk/respiratory-viruses/covid-19>
    - `variants/ch-wgs.csv` and `variants/ch-hosp.csv`: Switzerland,
      source: <https://covid19.admin.ch/>
