---
title: "Separating the forecaster from the target"
subtitle: "Model-based evaluation of forecast performance across a Forecast Hub"
author: "Kath Sherratt"
institute: "LSHTM · epiforecasts lab meeting"
date: last-modified
format:
  revealjs:
    slide-number: true
    chalkboard: true
    preview-links: auto
    footer: "Sherratt et al., medRxiv [10.1101/2025.04.10.25325611](https://doi.org/10.1101/2025.04.10.25325611)"
    css: lab-meeting.css
execute:
  echo: false
  warning: false
...
Question: does the structure of a forecasting model affect how well it forecasts?

## Two processes produce a score
...
::: {.smaller-text}
Every score is a prediction meeting a target. Separating the two splits the room a method has to improve from the accuracy any forecast can expect in that setting.

The target side is well described: stable dynamics, larger populations, timely surveillance and short horizons all help. The forecast side is not. Studies disagree on whether model structure matters, and a null result is hard to read when the two processes are mixed.
:::

## Forecast Hubs, set up to evaluate
...
[Relative WIS across models against a persistence baseline, 2 weeks ahead, 2021-2022. \* = ensemble. Sherratt et al. 2023, eLife.]{.caption}
:::
:::

## Crude performance: the target and horizon matter
...
## Teams choose their targets {.smaller}
...
::: notes
Click through the structures. Agent-based and judgement cover a handful of countries and weeks. Anything they "win" is a statement about those targets.
:::

## Why not stratify? {.smaller}
...
::: notes
Ask the room: which two covariates would you drop first? Try it live. Country and variant phase do the most damage.
:::

## The model {.smaller .model-slide}
...
Random effects shrink sparse groups towards the average. With `r n_models` models, the effective sample for structure is `r n_models`, not `r format(n_forecasts, big.mark = ",")`.

## Structure, before and after adjustment
...
Unadjusted, judgement looks `r round((1 - unadj("Judgement")) * 100)`% better than average. Adjusted, every structure sits between `r sprintf("%.2f", adj_range[1])` and `r sprintf("%.2f", adj_range[2])`, all intervals spanning 1.

::: notes
Judgement and agent-based are `r n_rare` of `r n_models` models and under 5% of forecasts.
:::

## What we wanted to see: structure by outcome
...
:::
::: {.column width="40%" .smaller-text}
> Differences between model structures were modified by epidemiological outcome. [...] An averaged estimate therefore gives close to zero for most structures, whether or not outcome-specific differences exist.
...
[Adjusted performance ratio (95% CI) against the average LWIS, for every categorical term except the 48 individual models. Highlighted: epidemic trend and variant phase. Deaths vs cases is a fixed contrast, ratio `r sprintf("%.2f", adj("Deaths"))`, not shown.]{.caption}

## Who looks best depends on the design {.smaller}
...
Spearman `r sprintf("%.2f", rank_summary$spearman)`. `r rank_summary$n_moved` of `r rank_summary$n` models move at least `r rank_summary$threshold` places; the furthest moves `r rank_summary$max_change`.

## Four questions we could not answer {.smaller}
...
::: {.card .fragment}
### Who turned up?
`r n_models` models is the effective sample; judgement and agent-based are `r n_rare`. One pathogen, one Hub, two years.

[Impact: wide intervals for the rare structures; no claim beyond this Hub.]{.impact}
:::
::: {.card .fragment}
### What did we adjust away?
Structure is a property of a model, and the model effect sits beside it. Horizon is fitted per model, so it can absorb structure × horizon.
...
::: notes
Spend time here. These are findings about Hubs as a source for evaluation, not only caveats on this analysis.
:::

## Limitations are a finding
...
- A stated estimand: which effect, adjusted for what, and why
:::

::: {.fragment}
Hubs are built to produce a good ensemble, and they do. They were not built to explain why individual forecasts are good.
:::

## Sensitivity: nothing we tried changed the result {.smaller}
...
::: {.step .theory}
[Theory 1]{.tag}
A score comes from a forecast and a target; unadjusted comparisons mix the two
:::
::: {.step .data}
[Data 1]{.tag}
Crude Hub scores mostly track the target and horizon
:::
::: {.step .theory}
[Theory 2]{.tag}
Designs run from unadjusted to formal; choose by the question
:::
::: {.step .data}
[Data 2]{.tag}
Teams pick targets; full stratification leaves 30% of strata filled
:::
::: {.step .theory}
[Theory 3]{.tag}
Regression uses every forecast, with target and forecaster terms side by side
:::
::: {.step .data}
[Data 3]{.tag}
No structure beats average; trend and variant shift scores about 20%; ranks move (ρ `r sprintf("%.2f", rank_summary$spearman)`)
:::
::: {.step .theory}
[Theory 4]{.tag}
Hubs lack the metadata, sampling and estimand this question needs
:::
::: {.step .data}
[Data 4]{.tag}
Nothing we tried changed the answer
:::
:::

[Built in Quarto from modular section files (`report/quarto/_*.qmd`), with every number computed from saved fits. This deck reuses the same code.]{.caption}

## What next? {.smaller}

1. Should Hubs require structured metadata (methods, revisions, team capacity)? Which few fields are worth the burden?
2. Should Hub leaderboards report adjusted scores alongside relative skill? Would teams accept a ranking built on a model?
3. Teams choose their targets. Is choosing hard targets a confounder to remove, or a skill to credit?
4. Next step in formality: propensity weights for participation, or pooling Hubs (US and Europe; COVID-19, flu, RSV) for more than 50 models?
5. If structure is a weak lever, what is worth measuring? Human judgement, handling of data revisions, update frequency, LLM assistance?
6. Would a prospective design help: quota recruitment across structures, or a core target set for everyone?
