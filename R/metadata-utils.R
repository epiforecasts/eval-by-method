...
get_model_names <- function(repo) {
  ...
  data_processed <- gh(paste0("/repos/", repo,
...
get_model_submissions <- function(repo = "covid19-forecast-hub-europe/covid19-forecast-hub-europe",
  ...
  model_content <- map(model_abbr,
                       ~ gh(paste0("/repos/", repo,
  ...
  model_submissions <- tibble("date" = model_dates) |>
    mutate(model = substr(date, 12, nchar(date)-4),
           date = floor_date(as.Date(substr(date, 1, 10)),
                             unit="week"))
  # Inclusion by date
  if (is.null(min_date)) {min_date <- min(model_submissions$date)}
  if (is.null(max_date)) {min_date <- max(model_submissions$date)}
...
get_metadata_raw <- function(model_abbr, repo) {
  ...
  {
    metadata_raw <- tryCatch(map_dfr(model_abbr,
                                     ~ read_yaml(paste0(
