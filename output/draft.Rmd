...
```{r ensembles, fig.height=6, fig.width=10}
# Ensemble variation scores by scale
ens_plot_all_scores <- scores_pairwise |> 
  filter(grepl("EuroCOVIDhub-ensemble", model) &
  ...
  arrange(rel_wis) |> 
  mutate(method_type = forcats::fct_inorder(method_type)) |> 
  pivot_wider(names_from = scale, values_from = rel_wis) |> 
...
# Combine plots
ens_plot_all <- ens_plot_all_scores +
  ens_plot_all_scores_time +
  patchwork::plot_annotation(tag_levels = "A") +
  patchwork::plot_layout(nrow = 1, widths = c(0.5, 1))

ggsave(plot = ens_plot_all, 
