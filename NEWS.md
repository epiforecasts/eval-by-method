...
## Unreleased — Model-building vignette

`R/model-building.qmd` (moved from `R/sensitivity/`), `R/analysis-model.R`

Rewrote `model-building.qmd` as a vignette narrating the GAMM building process step by step through the `analysis-model.R` functions (data preparation, family choice, unadjusted and joint fits, effect extraction, full pipeline).
Model-fitting chunks are shown but not evaluated; `R/analysis-model.R` remains the canonical pipeline for producing `output/`.
Also fixed `model_wis()`, which passed a leftover undefined `response` argument (from the removed log-response arm) to `prepare_model_data()` and `wis_family()`, making it error on every call.
