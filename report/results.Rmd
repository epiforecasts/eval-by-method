...
We explored the interval score (WIS) as a measure of predictive performance (Figure \@ref(fig:scores-over-time)), and characterised its association with model structure and number of countries targeted. We used descriptive summary statistics (the mean WIS), and then a generalised additive mixed model to give adjusted estimates of the partial effect of each factor while controlling for other sources of variation (Figure \@ref(fig:plot-coeffs)). The interval score was highly right-skewed with respect to all explanatory variables (see Supplementary Figure 1), which we accounted for by using a log-link.
...
table_effects <- map(random_effects, 
               ~ .x |> mutate(value_ci = paste0(round(value, 2), 
                           " (", round(lower_2.5, 2), "-", 
                           round(upper_97.5, 2), ")")) |> 
                 column_to_rownames("group"))
```

Descriptively, we noted apparently similar predictive performance between mechanistic and semi-mechanistic models. These performed relatively better than statistical models and worse than agent-based and "other" models. However, variation in performance overlapped between all model structures. Relative performance among modelling methods also appeared to vary over time (Figure \@ref(fig:scores-over-time)). For example, over summer 2021 all model types saw worsening performance coinciding with the introduction of the Delta variant across Europe, but this decline was most marked among statistical models of death outcomes compared to any other model type.

These descriptive differences largely disappeared after model-based adjustment. We found no clear evidence that any one type of model structure consistently outperformed others (Figure \@ref(fig:plot-coeffs)). There was no difference in accuracy between model structures when predicting cases, and we observed only weak differences when predicting deaths, indicating that statistical models may have performed slightly better (partial effect `r table_effects[["Deaths"]][["Statistical","value"]]`, 
95%CI `r table_effects[["Deaths"]][["Statistical","lower_2.5"]]`-`r table_effects[["Deaths"]][["Statistical","upper_97.5"]]`)), and semi-mechanistic models worse (`r table_effects[["Deaths"]][["Semi-mechanistic","value_ci"]]`) than the average, although with overlapping uncertainty.
...
In adjusted estimates, we also saw some indication that models focusing on a single country outperformed those modelling multiple countries (partial effect for single-country models forecasting cases: `r table_effects[["Cases"]][["Single-country","value_ci"]]` compared to `r table_effects[["Cases"]][["Multi-country","value_ci"]]` for multi-country models; and `r table_effects[["Deaths"]][["Single-country","value_ci"]]` and `r table_effects[["Deaths"]][["Multi-country","value_ci"]]` respectively when forecasting deaths). However, these effects were inconclusive with overlapping uncertainty.

<!--- Considering confounders --->

We considered the predictive horizon and epidemiological situation for each forecast as potentially confounding other associations with model performance. In unadjusted estimates, average performance worsened from 1 to 4 weeks of predictive horizon, and was best when the epidemiological situation was stable. Model based analysis supported this observation, albeit with overlapping confidence intervals. This indicated improved predictive performance during stable periods of each country's outbreak curve (cases: `r table_effects[["Cases"]][["Stable","value_ci"]]`; deaths: `r table_effects[["Deaths"]][["Stable","value_ci"]]`), compared to increasing trends in incidence (cases: `r table_effects[["Cases"]][["Increasing","value_ci"]]`; deaths: `r table_effects[["Deaths"]][["Increasing","value_ci"]]`), with worst performance seen when predicting decreasing trends (cases: `r table_effects[["Cases"]][["Decreasing","value_ci"]]`; deaths: `r table_effects[["Deaths"]][["Decreasing","value_ci"]]`). 
...
coeff_cap <- "Effects on model forecast performance (the weighted interval score). Explanatory variables included model structure (blue), and number of countries that each model contributed forecasts for (one or multiple countries, red). Partial effects and 95% confidence intervals estimated from fitting a generalised additive mixed model. A lower score indicates better performance, meaning effects <0 are relatively better than the group average."
